...
BOT_CERT_PATH = "/data/taky-bordercar/certs"
CERT_PATH = "/data/ssl"
HOSTNAME = "taky-bordercar"
...
def check_server_certs():
    if os.path.exists(CERT_PATH+'/ca.crt') == False:
        return False
    if os.path.exists(CERT_PATH+'/ca.key') == False:
        return False
    if os.path.exists(CERT_PATH+'/server.crt') == False:
        return False
    if os.path.exists(CERT_PATH+'/server.p12') == False:
        return False

    return True

def build_certs(cert_password):
    # Check if necessary Server certificates exist, wait until
    while check_server_certs() == False:
        print('Server certificates does not exist... Waiting')
        sleep(10)

    # Check if bot cert folder exist
    if os.path.exists(BOT_CERT_PATH):
        print('Bot Client Certificates exists, removing directory')
        shutil.rmtree(BOT_CERT_PATH)

    cdir = BOT_CERT_PATH
    os.makedirs(cdir)

    server_p12 = CERT_PATH + "/server.p12"

    # Build client certificates
    client_pkg_name = HOSTNAME
    make_cert(
        path=cdir,
        f_name=client_pkg_name,
        hostname=HOSTNAME,
        cert_pw=cert_password,
        cert_auth=(CERT_PATH + "/ca.crt", CERT_PATH + "/ca.key"),
        dump_pem=True,
    )
